;;; init-ref.el --- Bibliography and reference management -*- lexical-binding: t -*-

;; Set bibtex-completion-bibliography BEFORE any package loads it
...
(use-package ebib
  ...
  (defcustom ebib-database-path
    (expand-directory-name-auto-create "database" bibliography-directory)
    "The place where ebib reads BibTeX files.")
  (defun ebib-merge-bibtex-file-from-download ()
    "Merge a BibTeX file into the current database."
    (interactive)
    (let ((file (expand-file-name
                 (read-file-name "Import to merge: " eaf-webengine-download-path))))
      (unless (file-readable-p file)
        (error "[Ebib] No such file: %s" file))
      (let ((bib-file (convert-ris2bib file)))
        (ebib-import-entries bib-file nil)
        (when (and (not (string= bib-file file))
                   (y-or-n-p "Import complete. Delete source file? "))
          (delete-file file)))))
  :hook
...
|^^^^--------------------|-^^--------------|-^^-----------------|-^^---------------|-^^---------------|
| _s_,_S_: Save          | _a_: Add        | _/_: Search        | _<_: Ascending   | _uu_: Browse url |
|   ^^_w_: Save As       | _d_: Delete     | _?_: Search Next   | _>_: Descending  | _ud_: Browse doi |
...
(use-package oc
  ...
  (org-cite-csl-styles-dir
   (expand-directory-name-auto-create "styles" bibliography-directory)))

(use-package oc-bibtex
  :after oc
  :custom
  (org-cite-bibtex-bibliography-style "plain"))

(defhydra ref-hydra (:color blue :hint nil)
  "
 ^Reference Workflow^
^^^^^^^──────────────────────────────────────
 ^Manage^           ^Search^          ^Insert^
_e_ Ebib            _b_ Ivy Bibtex    _r_ Insert Link
_f_ Filters         ^ ^               ^ ^

 ^Notes^            ^Knowledge^       ^Other^
_n_ Interleave      _o_ Org Roam      _a_ Agenda
^ ^                 _d_ Dailies       _q_ Quit
"
  ("e" ebib)
  ("f" ebib-filters-hydra/body)
  ("b" ivy-bibtex)
  ("r" org-ref-insert-link-hydra/body)
  ("n" eaf-interleave-app-hydra/body)
  ("o" org-roam-hydra/body)
  ("d" org-roam-dailies-hydra/body)
  ("a" org-agenda-hydra/body)
  ("q" nil))

(global-set-key (kbd "C-c r") 'ref-hydra/body)
