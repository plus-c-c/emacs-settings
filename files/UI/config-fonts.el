...
(setq emacs-global-text-scale-factor
...
(defun cabins--font-setup (&optional default-fonts unicode-fonts emoji-fonts cjk-fonts)
  ...
  (when (display-graphic-p)
    (cabins--set-font-common nil (if default-fonts default-fonts cabins--fonts-default) emacs-global-text-scale-factor)
    (cabins--set-font-common 'unicode (if unicode-fonts unicode-fonts cabins--fonts-unicode) emacs-global-text-scale-factor)
    (cabins--set-font-common 'emoji (if emoji-fonts emoji-fonts cabins--fonts-emoji) emacs-global-text-scale-factor)
    (dolist (charset '(kana han bopomofo cjk-misc))
      (cabins--set-font-common charset (if cjk-fonts cjk-fonts cabins--fonts-cjk) emacs-global-text-scale-factor))))
(cabins--font-setup)
